...
namespace MyRecipeBook.API.Filter
{
    public class IdsFilter : IOperationFilter
    {
        ...
        {
            var encryptedIds = context.ApiDescription.ParameterDescriptions.Where(x => x.ModelMetadata.BinderType == typeof(MyRecipeBookIdBinder)).ToDictionary(d => d.Name, d => d);
